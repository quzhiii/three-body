...
|:---:|---|---|---|
| 🔭 **观机阵** | 分析、调研、理解、先看看 | risk=低, complexity=复杂, write=只读 | `brainstorming` / `review` |
| ⚡ **破局阵** | 实现、修复、重构、加功能 | risk=中, complexity=中等, write=代码 | `Code` / `test-runner` |
| 🔍 **明鉴阵** | 验证、review、检查、确认 | risk=低, complexity=中等, write=只读 | `review` / `qa` |
| 📝 **立言阵** | 写文档、总结、出方案 | risk=低, complexity=中等, write=文档 | `doc` / 文档类 skill |
| 🚀 **行令阵** | 部署、迁移、改配置、删除 | risk=高, write=配置/删除 | `careful` / `guard` |

下游 skill 的权威默认映射见 [`references/downstream-skills.md`](./references/downstream-skills.md)。
---
...
└── references/
    ├── downstream-skills.md       # 阵型 → 下游 skill 权威映射
...
根目录下的 `.skill` 文件是从源码目录生成的发布产物，不应手工编辑。
